var spawnCreeps = {
				...
				if (spawn.room.energyAvailable >= 850) {
					...
					if (
						spawn.spawnCreep(
							...
							'Harvester_Ex_' + roomDestination + number,
							{ dryRun: true }
						) === 0
					) {
						spawn.spawnCreep(
							[WORK, WORK, CARRY, CARRY, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE, MOVE],
							'Harvester_Ex_' + roomDestination + '_' + number,
							{
								memory: {
	...
	spawnHarvesterMinderal: function (
		spawn,
		roomDestination,
		creepsSpawned,
		creepsNeeded,
		dependingCreepsSpawned
	) {
		...
		if (controllerLevel >= 6) {
			...
			if (
				creepsSpawned < creepsNeeded &&
				room_extractor.length > 0 &&
				room_mineral[0].mineralAmount > 0 &&
				room_storageFull &&
				dependingCreepsSpawned > 0
			) {
				...
				if (
					spawn.spawnCreep(
						...
						'Harvester_M_' + rooms_around[x] + number,
						{ dryRun: true }
					) === 0
				) {
					spawn.spawnCreep(
						[WORK, WORK, WORK, WORK, WORK, CARRY, CARRY, MOVE, MOVE, MOVE, MOVE],
						'Harvester_M_' + rooms_around[x] + '_' + number,
						{
							memory: {
			...
			if (roomDestination.energyAvailable >= 1200) {
				if (
					spawn.spawnCreep(
						...
						'Builder_' + roomDestination.name + '_' + number,
						{ dryRun: true }
					) === 0
				) {
					spawn.spawnCreep(
			...
			if (spawn.room.energyAvailable >= 650) {
				...
				if (
					spawn.spawnCreep(
						...
						'Filler_' + roomDestination.name + '_' + number,
						{ dryRun: true }
					) === 0
				) {
					spawn.spawnCreep(
